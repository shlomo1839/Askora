...
export default function CreateSurvey() {
  ...
  useEffect(() => {
    ...
    const existingSurvey = StorageService.getSurveyById(surveyId);

    if (!existingSurvey) {
      setGeneralError('הסקר לא נמצא');
      return;
    }

    if (!StorageService.isSurveyOwner(surveyId, user.id)) {
    ...
    setCreatedAt(existingSurvey.createdAt);
  }, [isEditMode, surveyId]);
  ...
  const handleSubmit = () => {
    ...
    const finalSurveyId = isEditMode && surveyId ? surveyId : crypto.randomUUID();
    const survey = {
      id: finalSurveyId,
      title: title.trim(),
      ...
      createdAt: isEditMode && createdAt ? createdAt : new Date().toISOString(),
    };
    ...
    setSavedSurveyId(finalSurveyId);
    setDialogOpen(true);
    setTitle('');
    setDescription('');
      ...
      <Container maxWidth="md" sx={{ py: 4 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
          <Button variant="contained" size="large" onClick={handleSubmit}>
            {isEditMode ? 'שמירת שינויים' : 'פרסום הסקר'}
          </Button>
        </Box>
